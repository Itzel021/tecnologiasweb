...
if(isset($_GET['eliminado']))
    {
		$vigencia = $_GET['eliminado'];
    }
else
    {
        die('Parámetro "eliminado" no detectado...');
    }
    if (!empty($vigencia) || $vigencia == 0 || $vigencia == 1)
	{
		...
		if ( $result = $link->query("SELECT * FROM productos WHERE eliminado = $vigencia") ) 
		{
		...
		<table class="table">
			...
			<tbody>
				...
				foreach ($data as $key => $value) {
					...
					echo '<td><img src=' . $value['imagen'] . ' width="150px" height="150px" /></td>';
					echo '</tr>';
				}
				?>
